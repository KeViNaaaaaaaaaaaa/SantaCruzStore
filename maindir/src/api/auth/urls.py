...
from api.auth.views import register, user_login, profile, logout_user, profile_edit, activate, my_protected_view

urlpatterns = [
    path('register/', register, name='register'),
    re_path(r'^confirm-email/(?P<uidb64>[-\w]+)/(?P<token>[-\w]+)/$', activate, name='confirm_email'),
    path('login/', user_login, name='login'),
    path('my-protected-view/', my_protected_view, name='my_protected_view'),
